...
def radiometer_formula(alpha=2, tsys=None, freq_step=None,
    ...
    return np.sqrt(alpha) * tsys / (np.sqrt(np.abs(freq_step) * 1e6 * time)) * np.exp(tau / np.sin(elevation))
...
class SpectraStatistic(object):
    def __init__(self, index=None, alpha=2):
        self.index = index
        self.alpha = 2
    ...
    def get_statistics(self, factor=None):
        if len(np.unique(self.ry)) < 10:
            ...
            self.running_noise_evaluation(alpha=self.alpha)
    ...
    def cut_blanked_channels_on_either_side(self):
        ...
        self.number_of_blanked_channels = number_blanks
        print("Number of blanked channels: {}".format(self.number_of_blanked_channels))
    ...
    def running_noise_evaluation(self, avg_channels=40, alpha=2):
        pyclass.comm("get {}".format(self.index))
        self.min_max_difference = None
        self.cut_min_max_difference = None
        self.cut_blanked_channels_on_either_side()
        set_dummy_window()
        pyclass.comm("base 0")
        chunks = int(
        ...
        mean_tau = np.array(pyclass.gdict.r.assoc.tau.data.__sicdata__)
        mean_tau = mean_tau[mean_tau > -1e3].mean()

        mean_tsys = np.array(pyclass.gdict.r.assoc.tsys.data.__sicdata__)
        mean_tsys = mean_tsys[mean_tsys > -1e9].mean()

        try:
            min_value =  np.min(self.ry[(self.ry > -1e9) & (self.ry < 1e9)])
            max_value =  np.max(self.ry[(self.ry > -1e9) & (self.ry < 1e9)])
            self.min_max_difference = max_value - min_value
        except ValueError:
            pass

        try:
            min_value =  np.min(self.ry[(self.ry >- 1000) & (self.ry < 1000)])
            max_value =  np.max(self.ry[(self.ry >- 1000) & (self.ry < 1000)])
            self.cut_min_max_difference = max_value - min_value
        except ValueError:
            pass
    ...
    def generate_csv_entry(self):
        ...
        self.csv_entry["elevation"] = self.elevation
        ...
        self.csv_entry["number_blanked_channels"] = self.number_of_blanked_channels
        self.csv_entry["time"] = self.time
        self.csv_entry["min_max_difference"] = self.min_max_difference
        self.csv_entry["cut_min_max_difference"] = self.cut_min_max_difference
    ...
    def generate_all_blanks_csv(self):
        ...
        self.csv_entry["number_blanked_channels"] = None
        self.csv_entry["time"] = None
        self.csv_entry["min_max_difference"] = None
        self.csv_entry["cut_min_max_difference"] = None
