...
def get_class_windows():
    ...
    #
    windows = [window for window in zip(
        lower_range, upper_range) if window != (0.0, 0.0)]
    return windows


def get_index(observatory="",
              additional_meta_parameters=[],
              force_reload=False,
              get_rms=False,
              get_stats=False,
              get_fft_power=[],
              get_stats_over_range=False,
              input_file=None,
              output_file=None
              ):
    ...
    temporary_folder = ".tmp_{}".format(uuid_string)
    pyclass.message(pyclass.seve.i, "PD_INDEX", temporary_folder)
    if not os.path.exists(temporary_folder):
        subprocess.call(["mkdir", temporary_folder])
    pyclass.message(pyclass.seve.i, "PD_INDEX", temporary_folder)

    folder_context["script_dir"] = temporary_folder
    folder_context["memory_dir"] = temporary_folder
    config = {}
    config["general"] = {}
    ...
    # this is a hack for singularity that does not allow TTY
    try:
        filename = input_file
    except KeyError:
        filename = None
    ...
    if not filename:
        ...
        grabber.stop()
        filename = grabber.ttyData[0]
    # input_file = grabber.ttyData[0].split(":")[1].split("[")[0].strip()
    rex = re.match("Input file:\s(.+)\s+\[.+", filename)
    if rex is None:
        pyclass.message(pyclass.seve.i, "PD_INDEX",
                        "couldn't parse filename {0}".format(grabber.ttyData[0]))
        return
    else:
    ...
    pyclass.message(pyclass.seve.i, "PD_INDEX",
                    "input file {0}".format(input_file))
    ...
    if input_file.strip() == "none":
        pyclass.message(pyclass.seve.e, "PD_INDEX",
                        "No file loaded")
        return
        ...
        pyclass.message(pyclass.seve.e, "PD_INDEX",
                        "{0} file not found".format(input_file))
        return
    #
    pyclass.message(pyclass.seve.i, "PD_INDEX",
                    "Loading index from {} into pandas".format(input_file))
    new = True
    ...
    # try:
    if not output_file:
        output_file = "{1}/.pd_index_{0}.pkl".format(
            os.path.basename(input_file), output_path)
    if ((force_reload is False) and (
            round(
                previous_modification_times[os.path.basename(input_file)], 0
            ) >= (
                round(
                    modification_time, 0)
                )
        ...
        pyclass.message(pyclass.seve.i, "PD_INDEX",
                        "Loading from already created local table")
        df = pickle.load(
            open(output_file, "r")
            )
        new = False
        ...
        df = pd.df
    pyclass.message(pyclass.seve.i, "PD_INDEX",
                    "Pandas index loaded successfully")
    #
    #
    if not output_file:
        output_file = "{1}/.pd_index_{0}.pkl".format(
            ...
            output_path)
    if new:
        pickle.dump(pd.df, open(output_file, "wb"))
    previous_modification_times[os.path.basename(
        input_file)] = modification_time
    with open(modification_times_yaml_file, 'w') as outfile:
        yaml.dump(previous_modification_times, outfile)

    # remove the temporary folder
    subprocess.call(["rm", "-r", temporary_folder])
    return df
...
class ClassExecution(object):
    def run_class_script(self, script):
        if hasattr(self, "config"):
            try:
                batch_processing = self.config["pipeline"][
                    "class_processing"]["batch"]
            except KeyError:
...
class PandasClass(ClassExecution):
    def __init__(self,
        ...
        self.get_stats_over_range = get_stats_over_range
        if config is None:
            raise SystemExit(
        ...
        pyclass.message(pyclass.seve.i, "PCLASS",
                        "script: {}".format(template))
        pyclass.message(pyclass.seve.i, "PCLASS",
                        "Running Class script")
        ...
        standard_index = [
            "index", "number", "version", "telescope", "scan", "subscan",
            "boff", "loff", "line", "source"
        ]
        ...
        if observatory == "SOFIA_GREAT" and read_user_section:
            ...
            if len(self.aot_df) == 0:
                ...
                dummy_df["ut_minute"] = (
                    dummy_df["ut_minute"] - dummy_df["residual_minute"]
                    ).map(int)
        ...
        if additional_meta_parameters is not None:
            ...
            drop_columns = [column for column in [
                "cdobs", "utobs"] if column in self.df.columns]
            ...
            index_file = "{}/pandas_additional_meta.csv".format(
                folder_context["memory_dir"])
            #
            parameters_to_collect[parameters_to_collect.index(
                'R%HEAD%GEN%VER')] = "version"
            ...
            self.df = pd.merge(self.df, self.additional_df,
                               on=["number", "version"])

            # convert utobs column to timestamp
            check_utobs_cdobs_in_column = [column in self.df.columns for
                                           column in ["utobs", "cdobs"]]
            if all(check_utobs_cdobs_in_column):
                dummy_df = self.df.loc[:, ["utobs", "cdobs"]]
                # time
                dummy_df["ut_hour"] = (
                    np.floor(24 * dummy_df["utobs"] / (2 * np.pi)))
                dummy_df["ut_minute"] = np.floor(
                    (24 * dummy_df["utobs"] / (2 * np.pi) -
                     dummy_df["ut_hour"]) * 60.0)
                dummy_df["ut_second"] = (
                    (24 * dummy_df["utobs"] / (2 * np.pi) -
                     dummy_df["ut_hour"]) * 60 - dummy_df["ut_minute"]) * 60
        ...
        if get_stats:
            self._get_stats()
        if bool(get_stats_over_range):
            self._get_stats_over_range()
        #
    ...
    def _get_stats(self):
        ...
        self.window_string = " ".join(
            ["{0[0]} {0[1]}".format(window) for window in self.windows])
        ...
        #    index_file,
        #    names=["index","number","version",'stats_mean','stats_median',
        #           'stats_max','stats_min','stats_rms','stats_sum'])
        stats_df = pd.read_csv(
            index_file)
        stats_df.columns = stats_df.columns.str.strip()
        #
        if len(stats_df > 0):
            self.df = pd.merge(self.df, stats_df, on=[
    ...
    def _get_rms(self):
        ...
        if self.get_fft_power:
            ...
            #    self.fft_command += " /fft_frequency_focus {0}".format(
            #                                                          fft_freq)
            self.fft_definitions = ("def real fft_freqs[{0}] /global\n"
                                    "let fft_freqs {1}\n").format(
                len(self.get_fft_power), " ".join(self.get_fft_power))
        ...
        if len(self.rms_df > 0):
            self.df = pd.merge(self.df, self.rms_df, on=[
                               "index", "number", "version"])

    def _get_stats_over_range(self):
        #
        pyclass.message(
            pyclass.seve.e,
            "PD_INDEX",
            "get line int over window {0} to {1} km/s".format(
                min(self.get_stats_over_range),
                max(self.get_stats_over_range))
            )
        self.windows = get_class_windows()
        self.window_string = " ".join(
            ["{0[0]} {0[1]}".format(window) for window in self.windows])
        if self.window_string == "":
            msg = "no windows found, skipping get_stats_over_range collection"
            self.log.debug(msg)
            pyclass.message(pyclass.seve.e, "PD_INDEX", msg)
            return
        #
        template = ENV.get_template("pandas_get_stats_over_range.class")
        context = {
            "input_file": self.input_file,
            "memory_dir": self.folder_context["memory_dir"],
            "windows": self.window_string,
            "number_of_ranges":
                "def int stat_range_nb\nlet stat_range_nb {0}".format(
                    len(self.get_stats_over_range)),
            "units":
                "def char units[3]\nlet units {0}".format(
                    " ".join([stat_range[-1] for stat_range in
                              self.get_stats_over_range])),
            "lower_win":
                "def real lower_win[{0}]\nlet real lower_win {1}".format(
                    len(self.get_stats_over_range),
                    " ".join([min(stat_range[:-1]) for stat_range in
                              self.get_stats_over_range])),
            "upper_win":
                "def real upper_win[{0}]\nlet real upper_win {1}".format(
                    len(self.get_stats_over_range),
                    " ".join([max(stat_range[:-1]) for stat_range in
                              self.get_stats_over_range])),
            "max_win_int_win": max(self.get_stats_over_range),
            "min_win_int_win": min(self.get_stats_over_range)
        }
        script = ''.join(template.generate(context))

        script_name = "{}/get_stats_over_range.class".format(
            self.folder_context["script_dir"])
        with open(script_name, "w") as script_file:
            script_file.write(script)
        self.run_class_script(script_name)
        #
        index_file = "{}/get_stats_over_range.csv".format(
            self.folder_context["memory_dir"])
        #
        self.line_int_df = pd.read_csv(index_file)
        #
        print(self.line_int_df.columns)
        #
        if len(self.line_int_df > 0):
            self.df = pd.merge(self.df, self.line_int_df, on=[
                               "index", "number", "version"])
